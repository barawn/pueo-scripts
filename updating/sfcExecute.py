...
for s in surfList:
    jl = f'-u pyfwupd -o cat -n {args.lines}'
    print(f'Running journalctl {jl} on SURF#{surfAddrDict[s]}')
    l = hsk.journal(surfAddrDict[s], line=jl)
    msg1 = f'---------- eJournal {hex(surfAddrDict[s])} ----------'
    print(msg1)
    print(l)
    print('-'*len(msg1))
