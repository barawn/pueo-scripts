...
def surfStartup(tio,slots,enable=True,auto=True,commandonly=True,manual=False,
                douteye=-1,noturf=True,boring=True):
    TRAIN_WAIT_LOOPS = 40
    ALIGN_ATTEMPTS = 5
    ...
    douteye = None if douteye == -1 else douteye

    slotList = list(map(int,slots.split(',')))
...
if __name__=="__main__": 
    ...
    parser.add_argument("--douteye", type=int,default=-1,
                        help="use this specific dout eye")
    args = parser.parse_args()
    surfStartup(args.tio,args.slots,args.enable,args.auto,args.commandonly,args.manual,
                            args.douteye,args.noturf,args.boring)
